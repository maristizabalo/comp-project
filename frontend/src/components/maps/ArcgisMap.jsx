...
const ArcGISMapDraw = ({ geometry, onGeometryChange, required, error }) => {
    ...
    useEffect(() => {
        ...
        // Eventos del Sketch -> devolver al form en WGS84 (GeoJSON)
    ...
    // Pinta/centra cuando cambia `geometry` (desde Redux/Form)
    useEffect(() => {
        if (!graphicsLayer || !view) return;

        graphicsLayer.removeAll();

        if (!geometry) return; // nada que pintar
        ...
        <div className="flex flex-col h-full">
            <div className="flex flex-col space-y-2">
                <div className="flex space-x-2">
                    ...
                    <button type="button" onClick={clearGraphics} className="px-3 py-1 rounded bg-red-500 text-white">
                        Limpiar
                    </button>
                </div>
